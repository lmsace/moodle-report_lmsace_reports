 ...
 */
class siteresourceofnotvisit_table extends \table_sql implements dynamic_table {
    ...
    public function out($pagesize, $useinitialsbar, $downloadhelpbutton = '') {
        ...
        $countsql = "(SELECT COUNT(*) FROM {user} u WHERE NOT EXISTS (
            SELECT * FROM {logstore_standard_log} l WHERE l.userid = u.id AND l.timecreated >= :lastweek
        ) AND u.id > 2)";
        $this->set_count_sql($countsql, ['lastweek' => strtotime("-1week")]);
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        $from = "{user} u";
        $where = "NOT EXISTS (SELECT * FROM {logstore_standard_log} lg WHERE lg.userid = u.id AND
            lg.timecreated >= :lastweek) AND u.id > 2";
        $this->set_sql($select, $from, $where, ['lastweek' => strtotime("-1week")]);
